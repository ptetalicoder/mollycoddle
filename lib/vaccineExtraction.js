...
// Sends a picked PDF to the Mollycoddle backend (see ../backend) and gets
// back a best-effort list of { name, dateGiven, nextDueDate } extracted by
// Claude. This never touches the Anthropic API directly — the backend
// holds that key so it never ships inside the app.

import { File } from "expo-file-system";
import { VACCINE_BACKEND_URL, APP_SHARED_SECRET } from "./config";

export async function extractVaccinesFromPdf(documentUri) {
  const pdfBase64 = await new File(documentUri).base64();
