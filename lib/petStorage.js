...
// AsyncStorage only stores strings, so every pet needs a unique id to tell
// them apart. We don't have a real database generating ids for us, so we
// make one up from the current time + a random chunk.
export function makePetId() {
